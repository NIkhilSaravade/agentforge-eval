...
## Bug #7: `nvm install` resolves against the REMOTE index even when already installed — 31 real attempts corrupted by one network blip

**First sighting, initially misjudged as a one-off flake:** after resuming post-Bug-#6 fix, the watcher caught an `infra_error` at 527/960: `ollama_chat/qwen3:14b` / `colinhacks__zod-6532` / repeat 6 — `"could not resolve node 24 via nvm:\nVersion '24' not found - try nvm ls-remote to browse available versions."`, from `_node_bin_dir()`'s `nvm install {version} && nvm exec {version} corepack enable && nvm which {version}` script. Checked immediately: `nvm ls` and the exact same script, run by hand right after, both succeeded — node 24 was correctly installed and resolvable. Concluded (at the time, in good faith but wrongly) that this was a one-off flake needing no code fix, since `is_compile_failure()` correctly stayed `False` and `INFRA_ERROR` seemed like the right classification for a genuine environment hiccup. Removed the 1 bad line and resumed.

**It was not a one-off.** The very next check-in showed the failure had escalated to **30 consecutive `infra_error`s** — every single TS attempt for `qwen3:14b` from `date-fns__date-fns-3662` rep 0 onward (both date-fns instances × 10 repeats, then `trpc__trpc-7477` × 10 repeats), all with the exact same message, all sub-second (0.4–0.6s — consistent with a fast-failing shell call, not a real install taking real time).

**Root cause, found by reading `nvm.sh` itself rather than guessing:** `nvm install <version>` calls `nvm_remote_version()` — which does a real network fetch against nvm's remote version index — **before** it ever checks `nvm_is_version_installed()`. Confirmed directly in `$NVM_DIR/nvm.sh`: the remote-resolution call sits ahead of the local-install check in the function body. This means **every single call to `_node_bin_dir()`, for every one of hundreds of TS attempts, touched the network — even though node 24 had already been installed since the very first call.** A brief real network hiccup on this WSL2/Windows host during that specific window (never proven to be any one specific cause — WSL2's virtualized NAT, a transient DNS blip, whatever) was enough to make `nvm_remote_version()` fail to resolve the pattern `"24"` against an empty/failed remote list, which nvm then reports as `"Version '24' not found"` — a genuinely misleading error message for what's actually "the remote lookup failed," not "this version doesn't exist."

**Why the earlier manual re-checks (both the first flake's and this one's initial investigation) came back clean:** by the time each manual check ran, the transient network condition had already passed — a `nvm which <version>` (no network) or even a fresh `nvm install` (network, but now working again) both succeed once the blip is over. Confirmed by trying to reproduce on demand, twice (a tight 10-call loop, then a 30-call loop from one persistent Python process, matching `run_driver.py`'s own call pattern) — 40/40 succeeded outside the live run, both times. The bug is real (31 real corrupted attempts prove it) but not reproducible in isolation once the network condition that triggered it has resolved — exactly the profile of a genuine, if rare, transient network failure being needlessly exposed on every call instead of only the first.

**Fix:** `_node_bin_dir()` now checks locally first via `nvm which <version>` (reads only nvm's local `alias`/`versions` directory — no network at all) before ever falling back to the network-touching `nvm install` path. `corepack enable` is re-run unconditionally in the fast path too (it's local and idempotent — writes shims into that node install's own bin dir — cheap enough to just always ensure rather than track separately). The network-touching `nvm install` path only remains reachable for the genuinely-first call for a version that isn't installed yet, which is the only case where it's actually needed.

**Verification:** full test suite green (43/43); direct end-to-end re-run of `evaluate()` against `trpc__trpc-7477` with an empty patch now returns `status=ok` cleanly.
...
- [~] T10 — leaderboard page + methodology writeup, built against these real numbers instead of `MockModel` numbers. **Built and pushed (2026-09-19)**, generated from these result files; the outside-reproduction gate and the HuggingFace publish are still open (see the task board's status snapshot).
- [ ] `gpt-oss:20b` has never run, and `qwen3:14b` has 73 of 240 attempts left. Both are free but need many hours; not a paid-budget question.
- [ ] Repeats (k>1) for any paid model and a second paid model. The $10 budget is spent, so this needs an explicit new budget.
- [ ] Re-run `scripts/audit_dataset.py` on all 24 tasks (it last covered 13), and record each model's training cutoff so per-model contamination labels become possible.
- **Status of the 24-task set on 2026-09-19:** the human fix resolves 24 of 24 and an empty patch 0 of 24 through the runner; the results in this document are unchanged since the free tier was stopped at 647/960 and the paid tier at 20/24.
